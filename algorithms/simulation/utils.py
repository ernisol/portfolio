...
def latlon_to_xy(lat: float, lon: float, lat0: float, lon0: float) -> np.ndarray:
    """Convert latitude / longitude to x / y coordinates in an orthonormal plan centered in
    lat0, lon0."""
    lat = np.radians(lat)
    lon = np.radians(lon)
...
def xy_to_latlon(x: float, y: float, lat0: float, lon0: float) -> tuple[float, float]:
    """Convert x / y coordinates in an orthonormal plan centered in lat0, lon0 to
    latitude / longitude."""
...
def path_to_spline(
    ...
    # Filter path
    points_array = filter_path(points_array, min_dist=30)

    # Build spline
    x_spline, y_spline, s = build_spline(points_array)

    return x_spline, y_spline, s, lat0, lon0


def filter_path(points: np.ndarray, min_dist: float) -> np.ndarray:
...
def compute_arclength_params(points: np.ndarray) -> np.ndarray:
...
def evaluate_spline(x_spline: CubicSpline, y_spline: CubicSpline, s_vals: np.ndarray):
...
def resample_polyline(points, ds=50) -> np.ndarray:
